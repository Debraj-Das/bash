...
test_count_file="${file_name_without_extension}.num"
...
for ((i = 1; i <= test_cases; i++)); do
  input_file="${file_name_without_extension}${i}.in"
  output_file="${file_name_without_extension}${i}.res"
  expected_file="${file_name_without_extension}${i}.out"
